#from constants import EMPTY, OBSTACLE, SOURCE, TARGET, HEAD, NOOP, LEFT, LEFT, UP, RIGHT, DOWN
from jumanji.environments.combinatorial.routing.constants import SOURCE, TARGET, HEAD
...
@dataclass
class Vector:
    # Class of 2D tuple of ints, indicating the size of an array or a 2D vector in the array
    # this is way better than just having your usual class!
    x: int
    y: int

class Board:
    """ The boards are 2D numpy arrays of wiring routes on a printed circuit board.
    ...
    def __init__(self, x_dim: int, y_dim: int) -> None: 
        ...
        self.dim = Vector(x_dim, y_dim)
        self.num_wires = 0
        return None

    def get_random_head(self) -> Vector:
        # Return a random 2D vector, a starting point in the array
        x_dim = random.randint(0, self.dim.x-1) 
        y_dim = random.randint(0, self.dim.y-1) 
        return Vector(x_dim, y_dim)

    def get_wiring_directions(self, head: Vector) -> Tuple[Vector, Vector]:
        """ Return two orthogonal directions for the wire to go
        
        ARGS:
            head (Vector); The starting point of the wire
        Returns:
            (Vector) : The primary direction to run the wire
            (Vector) : The second direction to run the wire
        """
        ...
        if (head.y < self.dim.y/2):
            ...
            y_vector = Vector(0, -1)
        if max(head.x, self.dim.x-head.x) > max(head.y, self.dim.y-head.y):
    ...
    def is_full(self) -> bool:
        ...
        for i in range(self.dim.x):
            for j in range(self.dim.y):
                ...
                if (i < self.dim.x-1):
                    if (self.layout[i, j] == 0) and (self.layout[i+1, j] == 0):
                        return False
                if (j < self.dim.y-1):
                    if (self.layout[i, j] == 0) and (self.layout[i, j+1] == 0):
                        return False
    ...
    def add_connectible_cell(self, x_pos: int, y_pos: int, connectible_list: list) -> list:
        ...
        ARGS:
            ...
            connectible list (list[(int,int)...] : input list of connected cells
        RETURNS:
        ...
        if (x_pos < self.dim[0]-1) and (self.layout[x_pos+1, y_pos] == 0):
            self.add_connectible_cell(x_pos+1, y_pos, connectible_list)
        if (y_pos < self.dim[1]-1) and (self.layout[x_pos, y_pos+1] == 0):
            self.add_connectible_cell(x_pos, y_pos+1, connectible_list)
        if (x_pos > 0) and (self.layout[x_pos-1, y_pos] == 0):
            self.add_connectible_cell(x_pos-1, y_pos, connectible_list)
        if (y_pos > 0) and (self.layout[x_pos, y_pos-1] == 0):
            self.add_connectible_cell(x_pos, y_pos-1, connectible_list)
        return connectible_list
    ...
    def adjacent_empty_far_full(self, cell: (int,int)) -> bool:
        ...
        # If two neighboring adjacent cells are unconneted, it's a corner
        # If the opposit diagonal is connected, this corner is redundant.
...
def remove_connections(layout_in: np.ndarray) -> np.ndarray:
    ...
    ARGS:
        layout_in (2D numpy array of ints) : layout specifying heads, targets, and connectors
    RETURNS:
        layout_out (2D numpy array of ints) : same as layout_in with only heads and targets
    """
    layout_out = []
...
def board_generator(x_dim: int, y_dim: int, target_wires: int = None) -> Tuple[np.ndarray, np.ndarray, int]:
    ...
    Returns:
        board_training (numpy array of 2D integer tuples (x_dim, y_dim)) 
            Board design with head/target wiring ports marked
        board_solution (numpy array of 2D integer tuples (x_dim, y_dim)) 
