from jumanji.environments.combinatorial.routing.constants import TARGET, HEAD, \
    EMPTY
...
class DummyBoard:
    ...
    def get_wiring_directions(self, head: Position) -> Tuple[
        Position, Position]:
    ...
    def add_wire_random_walk(self, max_steps: int) -> None:
        ...
        while invalid_head == True:
            ...
            head = Position(random.randint(0, self.dim.x - 1),
        ...
        for step in range(max_steps):
            ...
            open_adjacent_cells = self.get_open_adjacent_cells(position,
                                                               wire_list)
        ...
        """ Returns a boolean, true if the cell is valid to add to the wire.
            ...
            Returns:
                bool: False if the cell is already in use,
                      ...
                      True, otherwise.
        """
        return (self.layout[input.x, input.y] == EMPTY) and (
        input.x, input.y) not in wire_list \
               and (self.number_of_adjacent_wires(input, wire_list) < 2)

    def get_open_adjacent_cells(self, input: Position,
                                wire_list: List) -> List:
        ...
        if input.x > 0 and self.is_valid_cell(Position(input.x - 1, input.y),
                                              wire_list):
            adjacent_list.append((input.x - 1, input.y))
        if input.y > 0 and self.is_valid_cell(Position(input.x, input.y - 1),
                                              wire_list):
            adjacent_list.append((input.x, input.y - 1))
        if input.x < self.dim.x - 1 and self.is_valid_cell(
                Position(input.x + 1, input.y), wire_list):
            adjacent_list.append((input.x + 1, input.y))
        if input.y < self.dim.y - 1 and self.is_valid_cell(
                Position(input.x, input.y + 1), wire_list):
            adjacent_list.append((input.x, input.y + 1))
        return adjacent_list

    def number_of_adjacent_wires(self, input: Position,
                                 wire_list: List) -> int:
    ...
    def add_connectible_cell(self, x_pos: int, y_pos: int,
                             connectible_list: List) -> List:
        ...
        if (x_pos < self.dim.x - 1) and (
                self.layout[x_pos + 1, y_pos] == EMPTY) and (
        x_pos + 1, y_pos) not in connectible_list:
            self.add_connectible_cell(x_pos + 1, y_pos, connectible_list)
        if (y_pos < self.dim.y - 1) and (
                self.layout[x_pos, y_pos + 1] == EMPTY) and (
        x_pos, y_pos + 1) not in connectible_list:
            self.add_connectible_cell(x_pos, y_pos + 1, connectible_list)
        if (x_pos > 0) and (self.layout[x_pos - 1, y_pos] == EMPTY) and (
        x_pos - 1, y_pos) not in connectible_list:
            self.add_connectible_cell(x_pos - 1, y_pos, connectible_list)
        if (y_pos > 0) and (self.layout[x_pos, y_pos - 1] == EMPTY) and (
        x_pos, y_pos - 1) not in connectible_list:
            self.add_connectible_cell(x_pos, y_pos - 1, connectible_list)
        return connectible_list

    def is_connectible(self, x_head: int, y_head: int, x_target: int,
                       y_target: int) -> bool:
    ...
    def add_wire_head_target_erode(self) -> None:
        ...
        while invalid_head:
            ...
            x_head, y_head = random.randint(0, self.dim.x - 1), random.randint(
                0, self.dim.y - 1)
            ...
            while not_done_removing:
                ...
                for cell in wire_list:
                    if self.three_sides_empty(cell, connectible_list) \
                            or self.is_extraneous_corner(cell,
                                                         connectible_list):
    ...
    def three_sides_empty(self, cell: (int, int),
                          connectible_list: List) -> bool:
    ...
    def is_extraneous_corner(self, cell: (int, int),
                             connectible_list: List) -> bool:
...
def board_generator(x_dim: int, y_dim: int, target_wires: int = None) -> (
...
def print_board(board_training: np.ndarray, board_solution: np.ndarray,
                num_wires: int) -> None:
...
if __name__ == "__main__":
    for i in range(9):
        x_dim, y_dim = 5, 5
        board_training, board_solution, num_wires = board_generator(x_dim,
                                                                    y_dim, i)
