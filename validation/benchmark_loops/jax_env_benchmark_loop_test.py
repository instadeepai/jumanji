...
from validation.benchmark_loops import JaxEnvBenchmarkLoop
...
@pytest.mark.parametrize("fake_jax_env", [()], indirect=True)
def test_jax_env_benchmark_run__does_not_smoke(
    fake_jax_env: FakeJaxEnv, capsys: pytest.CaptureFixture
) -> None:
    """Validates jax benchmark loop run method on a fake environment."""
    jax_env_benchmark_loop = JaxEnvBenchmarkLoop(fake_jax_env)
    jax_env_benchmark_loop.run(num_episodes=1)
