...
from jumanji.training.networks.bin_pack.transformer_block import TransformerBlock
...
class BinPackTorso(hk.Module):
    def __init__(
        ...
        super().__init__(name="bin_pack_torso")
...
def make_actor_critic_networks_bin_pack(
    ...
    num_ems = bin_pack.obs_num_ems

    parametric_action_distribution = FactorisedActionSpaceParametricDistribution(
        action_spec_num_values=np.asarray(bin_pack.action_spec().num_values)
    )
    policy_network = make_bin_pack_network(
    ...
    value_network = make_bin_pack_network(
        transformer_n_blocks=transformer_n_blocks,
        transformer_mlp_units=transformer_mlp_units,
        transformer_key_size=transformer_key_size,
        transformer_num_heads=transformer_num_heads,
...
def make_bin_pack_network(
    ...
    def network_fn(
        ...
        bin_pack_torso = BinPackTorso(
            transformer_n_blocks=transformer_n_blocks,
            transformer_mlp_units=transformer_mlp_units,
            transformer_key_size=transformer_key_size,
            transformer_num_heads=transformer_num_heads,
        )
        x = bin_pack_torso(observation)
