...
class GraphColoringTorso(hk.Module):
    ...
    def embed_nodes(self, nodes: chex.Array) -> chex.Array:
        ...
        nodes_leaves = jnp.stack(jtree.tree_leaves(nodes_float), axis=-1)
    ...
    def __call__(self, observation: Observation) -> chex.Array:
        ...
        color_indices = jnp.expand_dims(color_indices, axis=0)  # Shape (1, num_colors)
        color_indices = jnp.expand_dims(
            color_indices, axis=-1
        )  # Shape (1, num_colors, 1)
        ...
        color_indices = jnp.expand_dims(color_indices, axis=0)  # Shape (1, num_colors)
        color_indices = jnp.expand_dims(
            color_indices, axis=0
        )  # Shape (1, 1, num_colors)
