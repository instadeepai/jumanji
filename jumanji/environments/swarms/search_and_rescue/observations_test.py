...
def test_search_and_target_view_searchers(
    ...
    observe_fn = observations.AgentAndTargetObservationFn(
        num_vision=11,
        searcher_vision_range=VISION_RANGE,
        target_vision_range=VISION_RANGE,
    ...
    assert obs.shape == (n_agents, 3, observe_fn.num_vision)

    expected = jnp.full((n_agents, 3, observe_fn.num_vision), -1.0)
