...
@pytest.fixture
def bin_pack_random_select_action(bin_pack_env: BinPack) -> SelectActionFn:
    num_ems, num_items = bin_pack_env.action_spec().num_values
...
def test_bin_pack__reset(bin_pack_env: BinPack) -> None:
    """Validates the jitted reset of the environment."""
    chex.clear_trace_counter()
    reset_fn = jax.jit(chex.assert_max_traces(bin_pack_env.reset, n=1))
...
def test_bin_pack_step__jit(bin_pack_env: BinPack) -> None:
    """Validates jitting the environment step function."""
    chex.clear_trace_counter()
    step_fn = jax.jit(chex.assert_max_traces(bin_pack_env.step, n=1))

    key = jax.random.PRNGKey(0)
    state, timestep = bin_pack_env.reset(key)

    action = bin_pack_env.action_spec().generate_value()
...
def test_bin_pack__render_does_not_smoke(
    bin_pack_env: BinPack, dummy_instance: State
) -> None:
    bin_pack_env.render(dummy_instance)
    bin_pack_env.close()


def test_bin_pack__does_not_smoke(
    bin_pack_env: BinPack,
    bin_pack_random_select_action: SelectActionFn,
) -> None:
    """Test that we can run an episode without any errors."""
    check_env_does_not_smoke(bin_pack_env, bin_pack_random_select_action)


def test_bin_pack__pack_all_items_dummy_instance(
    bin_pack_env: BinPack, bin_pack_random_select_action: SelectActionFn
) -> None:
    ...
    while not timestep.last():
        ...
        state, timestep = jax.jit(bin_pack_env.step)(state, action)
...
def test_bin_pack__optimal_policy_toy_instance(
    ...
    solution = toy_bin_pack.instance_generator.generate_solution(key)
...
def test_bin_pack__optimal_policy_random_instance(
    ...
    generate_solution_fn = jax.jit(random_bin_pack.instance_generator.generate_solution)
