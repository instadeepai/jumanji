...
from jumanji.environments.logic.game2048.types import Board, State
...
def test_env_reset_jit(game2048: Game2048) -> None:
    """Confirm that the reset method is only compiled once when jitted."""
    chex.clear_trace_counter()
    reset_fn = jax.jit(chex.assert_max_traces(game2048.reset, n=1))
...
def test_env_step_jit(game2048: Game2048) -> None:
    ...
    chex.clear_trace_counter()
    step_fn = jax.jit(chex.assert_max_traces(game2048.step, n=1))
...
def test_add_random_cell(game2048: Game2048, board: Board) -> None:
    """Validate that add_random_cell places a 1 or 2 in an empty spot on the board."""
    key = jax.random.PRNGKey(0)
    add_random_cell = jax.jit(game2048._add_random_cell)
...
def test_get_action_mask(game2048: Game2048, board: Board) -> None:
    """Verify that the action mask generated by `get_action_mask` is correct."""
    action_mask_fn = jax.jit(game2048._get_action_mask)
...
def test_2048__does_not_smoke(game2048: Game2048) -> None:
    """Test that we can run an episode without any errors."""
    check_env_does_not_smoke(game2048)
