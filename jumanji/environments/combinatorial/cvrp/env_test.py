...
def test_cvrp__reset(cvrp_env: CVRP) -> None:
    ...
    reset_fn = jax.jit(cvrp_env.reset)
    ...
    assert state.visited_mask[DEPOT_IDX] == 1
    ...
    assert state.problem[DEPOT_IDX, 2] == 0
...
def test_cvrp__does_not_smoke(cvrp_env: CVRP, capsys: pytest.CaptureFixture) -> None:
...
def test_tsp__trajectory_action(cvrp_env: CVRP) -> None:
    ...
    while not timestep.last():
        ...
        if pending_position is not None:
            ...
            if state.capacity < state.problem[next_position, 2]:
...
def test_cvrp__tour_length() -> None:
    ...
    tour_length = compute_tour_length(problem, order)
    assert tour_length == 6.8649917

    # Check augmentations have same tour length
    augmentations = get_augmentations(problem)
    lengths = jax.vmap(compute_tour_length, in_axes=(0, None))(augmentations, order)
    assert jnp.allclose(lengths, jnp.ones(augmentations.shape[0], dtype=jnp.float32) * tour_length)

    order = jnp.array([0, 7, 8, 9, 10, 0, 1, 2, 3, 4, 5, 0, 6])
    assert compute_tour_length(problem, order) == 6.8649917
