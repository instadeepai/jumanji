...
class Board:
    ...
    def add_connectible_cell(self, x_pos: int, y_pos: int, connectible_list: List) -> List:
        ...
        if (x_pos < self.dim[0]-1) and (self.layout[x_pos+1, y_pos] == EMPTY):
            self.add_connectible_cell(x_pos+1, y_pos, connectible_list)
        if (y_pos < self.dim[1]-1) and (self.layout[x_pos, y_pos+1] == EMPTY):
            self.add_connectible_cell(x_pos, y_pos+1, connectible_list)
        if (x_pos > 0) and (self.layout[x_pos-1, y_pos] == EMPTY):
            self.add_connectible_cell(x_pos-1, y_pos, connectible_list)
        if (y_pos > 0) and (self.layout[x_pos, y_pos-1] == EMPTY):
            self.add_connectible_cell(x_pos, y_pos-1, connectible_list)
        return connectible_list
