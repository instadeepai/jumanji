...
class TestObservationSpec:
    @pytest.fixture
    def env(self) -> CoopMinSpanTree:
        """Instantiates a default CoopMinSpanTree environment."""
        return CoopMinSpanTree()

    @pytest.fixture
    def observation_spec(self, env: CoopMinSpanTree) -> ObservationSpec:
    ...
    def test_observation_spec__generate_value(self, env: CoopMinSpanTree) -> None:
    ...
    def test_observation_spec__validate_shape(
        ...
        modified_shape_observation = observation._replace(
            node_types=observation.node_types[None, ...]
        )
    ...
    def test_observation_spec__validate_dtype(
        ...
        modified_dtype_observation = observation._replace(
            node_types=observation.node_types.astype(jnp.float16)
        )
        with pytest.raises(ValueError):
    ...
    def test_observation_spec__replace(self, observation_spec: ObservationSpec) -> None:
        ...
        args = [
            (
                "node_types_spec",
                observation_spec.node_types_spec.replace(shape=(3, 4)),
            ),
            ("edges_spec", observation_spec.edges_spec.replace(shape=(3, 4))),
            (
                "position_spec",
                observation_spec.position_spec.replace(name="new_name"),
            ),
            (
                "action_mask_spec",
