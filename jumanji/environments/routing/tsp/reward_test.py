...
def test_dense_reward(tsp_dense_reward: TSP, dense_reward: DenseReward) -> None:
    ...
    for action in range(tsp_dense_reward.num_cities):
        next_state, _ = tsp_dense_reward.step(state, action)
        assert dense_reward(state, action, next_state) == 0

    # Check that the reward is correct for the next city
    state, timestep = tsp_dense_reward.step(state, 0)
    for action in range(1, tsp_dense_reward.num_cities):
        ...
        assert dense_reward(state, action, next_state) == -distance

    # Check the reward for invalid action
    next_state, _ = tsp_dense_reward.step(state, 0)
    penalty = -jnp.sqrt(2) * tsp_dense_reward.num_cities
    assert dense_reward(state, 0, next_state) == penalty
...
def test_sparse_reward(  # noqa: CCR001
    ...
    while not timestep.last():
        for action, is_valid in enumerate(timestep.observation.action_mask):
            if is_valid:
                next_state, timestep = jax.jit(tsp_sparse_reward.step)(state, action)
                if timestep.last():
                    ...
                    assert sparse_reward(state, action, next_state) == -tour_length
                else:
                    # Check that the reward is 0 for every non-final valid action
                    assert sparse_reward(state, action, next_state) == 0
            else:
                # Check that a penalty is given for every invalid action
                invalid_next_state, _ = jax.jit(tsp_sparse_reward.step)(state, action)
                assert sparse_reward(state, action, invalid_next_state) == penalty
        state = next_state
